...
from .logging_service import send_log_to_elasticsearch
...
def fetch_sku_data():
    ...
    try:
        ...
        query = "SELECT * FROM view_sku_dataset"
        ...
        client.close()
        logger.info("Dados de SKU buscados com sucesso.")
        log_message.append("Dados de SKU buscados com sucesso. \n")
        
        status_code = 200
        return df
    except Exception as e:
        logger.error(f"Erro ao buscar dados de SKU: {str(e)}", exc_info=True)
        log_message.append(f"Erro ao buscar dados de SKU: {str(e)} \n")

        status_code = 500
        raise RuntimeError("Falha ao buscar dados de SKU") from e
    finally:
        send_log_to_elasticsearch(log_message, "fetch_sku_data", status_code)
...
def fetch_stock_data():
    ...
    try:
        ...
        client.close()
        logger.info("Dados de estoque buscados com sucesso.")
        log_message.append("Dados de estoque buscados com sucesso. \n")
        
        status_code = 200
        return df
    except Exception as e:
        logger.error(f"Erro ao buscar dados de estoque: {str(e)}", exc_info=True)
        log_message.append(f"Erro ao buscar dados de estoque: {str(e)} \n")

        status_code = 500
        raise RuntimeError("Falha ao buscar dados de estoque") from e
    finally:
        send_log_to_elasticsearch(log_message, "fetch_stock_data", status_code)
