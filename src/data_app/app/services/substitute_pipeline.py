...
def process_substitute_data_from_s3():
    log_message = []

    try:
        prefix = os.getenv('PREFIX_SUBSTITUTES')
        ...
        if not csv_files:
            logger.warning("Nenhum arquivo CSV de substitutos encontrado no bucket S3")
            log_message.append("Nenhum arquivo CSV de substitutos encontrado no bucket S3 \n")
        ...
        for csv_file in csv_files:
            try:
                ...
                table_name = 'product_substitutes'  # Defina o nome da tabela de substitutos
                logger.info(f"Inserindo dados na tabela: {table_name}")
                log_message.append(f"Inserindo dados na tabela: {table_name} \n")
                ...
                client = get_client()
                insert_dataframe(client, table_name, df_prepared)
                logger.info(f"DataFrame inserido na tabela '{table_name}' no ClickHouse")
                log_message.append(f"DataFrame inserido na tabela '{table_name}' no ClickHouse \n")
        ...
        status_code = 200
        return jsonify({"message": "Dados de substitutos processados e inseridos no ClickHouse com sucesso!"}), status_code
        ...
        send_log_to_elasticsearch(log_message, "process_substitute_data_from_s3", status_code)
